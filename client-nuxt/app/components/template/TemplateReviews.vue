...
// Используем useDatesApi вместо @/store и @/services/api
const { getDateReviews, markReviewHelpful } = useDatesApi()
...
const displayedReviews = computed(() => filteredReviews.value.slice(0, displayCount.value))
const hasMore = computed(() => displayedReviews.value.length < filteredReviews.value.length)
...
.rating-count { margin-top: 8px; color: #718096; font-size: 0.9rem; }

.rating-distribution { display: flex; flex-direction: column; gap: 8px; }

.distribution-bar { display: flex; align-items: center; gap: 12px; }

.star-label { width: 50px; font-size: 0.9rem; color: #4a5568; font-weight: 600; }
...
.bar-count { width: 40px; text-align: right; font-size: 0.85rem; color: #718096; }
...
.reviews-loading { text-align: center; padding: 60px 20px; }
...
.reviews-empty { text-align: center; padding: 60px 20px; color: #718096; }
...
.add-review-hint { color: #718096; margin: 0 0 20px 0; }
...
@media (max-width: 768px) {
  ...
  .template-reviews { padding: 32px 20px; }
}
</style>