<template>
  <div class="qe">
    <header class="qe-header">
      ...
      <h1 class="qe-header__title">Новый квест</h1>
    ...
    <transition name="qe-toast-slide">
      <div v-if="toast" class="qe-toast" :class="`qe-toast--${toast.type}`">{{ toast.msg }}</div>
    </transition>
  </div>
</template>
...
if (!auth.isAuthenticated) await navigateTo('/admin/login')
